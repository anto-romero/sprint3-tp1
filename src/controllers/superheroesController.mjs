import {
  ...
  eliminarSuperheroe, eliminarSuperheroePorNombre
...
export async function crearNuevoSuperheroeController(req, res) {
  try {
    ...
    res.send(renderizarSuperheroe(newHero));
  } catch (error) {
...
export async function eliminarSuperheroeController(req, res) {
  try {
    ...
    res.status(200).send(`Superheroe con ID ${id}: ${JSON.stringify(deletedhero)}`);
  } catch (error) {
    res.status(500).send(error.message);
  }
}
...
export async function eliminarSuperheroePorNombreController(req,res) {
  try {
    ...
    res.status(200).send(`Superheroe con nombre: ${nombre}, ha sido eliminado ${JSON.stringify(deleteHeroByName)}`);
